{% macro spatial_join_with_deduplication(left_model, right_model, left_cols, right_cols, left_geom="geometry", right_geom="geometry", op="st_intersects", kind="left") %}

with left_model_with_id as (
    select
...
joined as (
    select
      ...
      left_model_with_id.{{ lcol }},
      {% endfor %}
      {% for rcol in right_cols %}
      {{ right_model }}.{{ rcol }},
      {% endfor %}
      left_model_with_id.{{ left_geom }},
      ...
      st_area(
        st_intersection(left_model_with_id.{{ left_geom }}, {{ right_model }}.{{ right_geom }})
      ) as _tmp_sjoin_intersection,
      left_model_with_id._tmp_sjoin_id
    from left_model_with_id
    {{ kind }} join {{ right_model }}
    on {{ op }}(left_model_with_id.{{ left_geom }}, {{ right_model }}.{{ right_geom }})
),
