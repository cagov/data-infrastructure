...
GitHub's [Dependabot](https://github.com/dependabot) service provides automated security scanning
of dependencies to repositories hosted in GitHub.
It is enabled by default in the `cagov` organization.

## Why Dependabot?

Automated security scans, combined with practices like cooldowns, allow for a relatively
painless way to keep on top of security vulnerabilities in our software dependencies.
...
1. Use dependency cooldowns. [Dependency cooldowns](https://docs.astral.sh/uv/concepts/resolution/#dependency-cooldowns)
    are a very simple, but seemingly very effective, way to avoid pulling in compromised
...
However, we still need to enable pull request grouping to avoid getting one
...
you can either merge the auto-generated pull request, or create
your own pull request that updates dependencies.
...
Once those are configured, maintainers could simply click "Allow"
on Dependabot security bump PRs, and then CI would run on the Dependabot
PR directly, followed by merge if it passes.
